...
download_send_headers("grup_istatistik_baslangic_" . $_GET["startDate"] . "_bitis_" . $_GET["endDate"] . "_grupID_" . $_GET["groupId"] . ".csv");
$data = array();
$db = $meridyen->db;
...
$contractCountStatement = $db->prepare("SELECT u.userId, count(c.contractId) FROM contract c,users u WHERE u.sectorId=? AND contractDate>=? 
	AND contractDate <=? AND confirmed = 1 AND cancelled = 0 AND c.customerRepresentativeId=u.userId GROUP BY u.userId");
$contractCountStatement->bind_param("dss", $_GET["groupId"], $startDateMerged, $endDateMerged);
...
$cancelledContractCountStatement = $db->prepare("SELECT u.userId, count(c.contractId) FROM contract c,users u WHERE u.sectorId=? AND contractDate>=? 
	AND contractDate <=? AND cancelled = 1 AND c.customerRepresentativeId=u.userId GROUP BY u.userId");
$cancelledContractCountStatement->bind_param("dss", $_GET["groupId"], $startDateMerged, $endDateMerged);
...
$gorusmeCountStatement = $db->prepare("SELECT u.userId,count(meetingId) FROM customermeeting c, users u WHERE u.sectorId=?
...
$gorusmeCountStatement->bind_param("dss", $_GET["groupId"], $startDateMerged, $endDateMerged);
...
$randevuCountStatement = $db->prepare("SELECT u.userId,count(meetingId) FROM customermeeting c, users u WHERE u.sectorId=?
	AND u.userId=c.customerRepresentativeId AND c.meetingType=1 AND meetingDate>=? 
	AND meetingDate <=? AND c.customerRepresentativeId=u.userId GROUP BY u.userId");
$randevuCountStatement->bind_param("dss", $_GET["groupId"], $startDateMerged, $endDateMerged);
...
$data["satism2"] = array();
$contractStandAreaStatement = $db->prepare("SELECT u.userId,SUM(standArea) FROM contract c, users u WHERE u.sectorId=?
...
$contractStandAreaStatement->bind_param("dss", $_GET["groupId"], $startDateMerged, $endDateMerged);
...
$cancelledContractStandAreaStatement = $db->prepare("SELECT u.userId,SUM(standArea) FROM contract c, users u WHERE u.sectorId=?
	AND c.customerRepresentativeId = u.userId AND contractDate>=? AND contractDate <=? AND c. cancelled = 1 GROUP BY u.userId");
$cancelledContractStandAreaStatement->bind_param("dss", $_GET["groupId"], $startDateMerged, $endDateMerged);
...
$contractPriceStatement = $db->prepare("SELECT u.userId,SUM(contractAmountWithKdv) FROM contract c, users u WHERE u.sectorId=?
	AND c.customerRepresentativeId = u.userId AND contractDate>=? AND contractDate <=? AND c.confirmed = 1 AND c. cancelled = 0 GROUP BY u.userId");
$contractPriceStatement->bind_param("dss", $_GET["groupId"], $startDateMerged, $endDateMerged);
...
$cancelledContractPriceStatement = $db->prepare("SELECT u.userId,SUM(contractAmountWithKdv) FROM contract c, users u WHERE u.sectorId=?
	AND c.customerRepresentativeId = u.userId AND contractDate>=? AND contractDate <=? AND c. cancelled = 1 GROUP BY u.userId");
$cancelledContractPriceStatement->bind_param("dss", $_GET["groupId"], $startDateMerged, $endDateMerged);
...
$contractDiscountAmountStatement = $db->prepare("SELECT u.userId,SUM(discountAmount) FROM contract c, users u WHERE u.sectorId=?
	AND c.customerRepresentativeId = u.userId AND contractDate>=? AND contractDate <=? AND c.confirmed = 1 AND c. cancelled = 0 GROUP BY u.userId");
$contractDiscountAmountStatement->bind_param("dss", $_GET["groupId"], $startDateMerged, $endDateMerged);
...
$groupDescriptionStatement = $db->prepare("SELECT description FROM sector where sectorId=?");
$groupDescriptionStatement->bind_param("d", $_GET["groupId"]);
$groupDescription = "";
if($groupDescriptionStatement->execute())
{
	$groupDescriptionStatement->store_result();
	$groupDescriptionStatement->bind_result($groupDescription);
	$groupDescriptionStatement->fetch();
}
$groupDescriptionStatement->free_result();
...
foreach($mtNames as $mtName)
{
	...
	$realData[] = array($_GET["groupId"], $groupDescription, $mtName, $mtRealName, $sozlesmesayi, $iptalsozlesmesayi, $gorusmesayi, $randevusayi, $satism2, $iptalSatism2, 
		$satisFiyat, $iptalSatisFiyat, $indirimTutar);
	
}
//var_dump($realData);
$arrHeader = array("Grup ID", "Grup İsmi", "MT ID", "MT İsim", "Sözleşme Sayı", "İptal Edilen Sözleşme Sayı", "Görüşme Sayı", "Randevu Sayı", "Satış (m2)",
