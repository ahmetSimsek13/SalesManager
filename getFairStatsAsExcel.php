...
download_send_headers("fuar_istatistik_baslangic_" . $_GET["startDate"] . "_bitis_" . $_GET["endDate"] . "_fuarID_" . $_GET["fairId"] . ".csv");
$data = array();
$db = $meridyen->db;
...
$contractCountStatement = $db->prepare("SELECT u.userId, count(c.contractId) FROM contract c,users u WHERE c.fairId=? AND contractDate>=? 
	AND contractDate <=? AND confirmed = 1 AND cancelled = 0 AND c.customerRepresentativeId=u.userId GROUP BY u.userId");
$contractCountStatement->bind_param("dss", $_GET["fairId"], $startDateMerged, $endDateMerged);
...
$cancelledContractCountStatement = $db->prepare("SELECT u.userId, count(c.contractId) FROM contract c,users u WHERE c.fairId=? AND contractDate>=? 
	AND contractDate <=? AND cancelled = 1 AND c.customerRepresentativeId=u.userId GROUP BY u.userId");
$cancelledContractCountStatement->bind_param("dss", $_GET["fairId"], $startDateMerged, $endDateMerged);
...
$gorusmeCountStatement = $db->prepare("SELECT u.userId,count(meetingId) FROM customermeeting c, users u WHERE c.fairId=?
...
$gorusmeCountStatement->bind_param("dss", $_GET["fairId"], $startDateMerged, $endDateMerged);
...
$randevuCountStatement = $db->prepare("SELECT u.userId,count(meetingId) FROM customermeeting c, users u WHERE c.fairId=?
	AND u.userId=c.customerRepresentativeId AND c.meetingType=1 AND meetingDate>=? 
	AND meetingDate <=? AND c.customerRepresentativeId=u.userId GROUP BY u.userId");
$randevuCountStatement->bind_param("dss", $_GET["fairId"], $startDateMerged, $endDateMerged);
...
$data["satism2"] = array();
$contractStandAreaStatement = $db->prepare("SELECT u.userId,SUM(standArea) FROM contract c, users u WHERE c.fairId=?
...
$contractStandAreaStatement->bind_param("dss", $_GET["fairId"], $startDateMerged, $endDateMerged);
...
$cancelledContractStandAreaStatement = $db->prepare("SELECT u.userId,SUM(standArea) FROM contract c, users u WHERE c.fairId=?
	AND c.customerRepresentativeId = u.userId AND contractDate>=? AND contractDate <=? AND c. cancelled = 1 GROUP BY u.userId");
$cancelledContractStandAreaStatement->bind_param("dss", $_GET["fairId"], $startDateMerged, $endDateMerged);
...
$contractPriceStatement = $db->prepare("SELECT u.userId,SUM(contractAmountWithKdv) FROM contract c, users u WHERE c.fairId=?
	AND c.customerRepresentativeId = u.userId AND contractDate>=? AND contractDate <=? AND c.confirmed = 1 AND c. cancelled = 0 GROUP BY u.userId");
$contractPriceStatement->bind_param("dss", $_GET["fairId"], $startDateMerged, $endDateMerged);
...
$cancelledContractPriceStatement = $db->prepare("SELECT u.userId,SUM(contractAmountWithKdv) FROM contract c, users u WHERE c.fairId=?
	AND c.customerRepresentativeId = u.userId AND contractDate>=? AND contractDate <=? AND c. cancelled = 1 GROUP BY u.userId");
$cancelledContractPriceStatement->bind_param("dss", $_GET["fairId"], $startDateMerged, $endDateMerged);
...
$contractDiscountAmountStatement = $db->prepare("SELECT u.userId,SUM(discountAmount) FROM contract c, users u WHERE c.fairId=?
	AND c.customerRepresentativeId = u.userId AND contractDate>=? AND contractDate <=? AND c.confirmed = 1 AND c. cancelled = 0 GROUP BY u.userId");
$contractDiscountAmountStatement->bind_param("dss", $_GET["fairId"], $startDateMerged, $endDateMerged);
...
$fairNameStatement->bind_param("d", $_GET["fairId"]);
$fairName = "";
...
foreach($mtNames as $mtName)
{
	...
	$realData[] = array($_GET["fairId"], $fairName, $mtName, $mtRealName, $sozlesmesayi, $iptalsozlesmesayi, $gorusmesayi, $randevusayi, $satism2, $iptalSatism2, 
		$satisFiyat, $iptalSatisFiyat, $indirimTutar);
	
}
//var_dump($realData);
$arrHeader = array("Fuar ID", "Fuar İsmi", "MT ID", "MT İsim", "Sözleşme Sayı", "İptal Edilen Sözleşme Sayı", "Görüşme Sayı", "Randevu Sayı", "Satış (m2)",
